...
export const SkillModel = {
  ...
  async create(input: SkillInput): Promise<Skill> {
    ...
    const builtIn = input.built_in !== undefined ? (input.built_in ? 1 : 0) : 0;
    await query(
      'INSERT INTO agent_skills (id, name, display_name, description, system_prompt, tags, built_in) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, input.name, input.display_name, input.description || null, input.system_prompt, tags, builtIn]
    );
  ...
  async update(id: string, input: Partial<SkillInput>): Promise<void> {
    ...
    if (input.built_in !== undefined) { fields.push('built_in = ?'); values.push(input.built_in ? 1 : 0); }
