...
/** 如果 webhook 端点未配置 tools，则自动使用所有已启用的工具 */
...
router.post('/:token', async (req: Request, res: Response) => {
  try {
    ...
    // 解析工具列表（未配置则自动使用所有已启用工具）
    const resolvedTools = await resolveDefaultTools(endpoint.selected_tools);
    const resolvedSkills = endpoint.selected_skills || [];

    // 创建执行记录
    const execution = await executionService.create(endpoint.session_id, safeMessage ?? '');

    // 始终使用流式执行（SSE 事件会被缓冲，前端连接后可重放）
    runAgentStreaming({
