...
-- 添加 file_path 字段（如果不存在）
ALTER TABLE agent_skills ADD COLUMN IF NOT EXISTS file_path VARCHAR(500) DEFAULT NULL COMMENT '本地 SKILL.md 文件路径';
...
-- 内置技能
INSERT IGNORE INTO agent_skills (id, name, display_name, description, system_prompt, enabled) VALUES
  ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'general_assistant', '通用助手', '通用对话助手，适合日常问答',
   '你是一个智能助手，请用中文回答用户的问题。你的回答应该准确、有帮助、简洁。', 1),
