...
function ChatMessage({ role, content, timestamp, receivedAt, completedAt, source }: Props) {
  ...
  const formatTime = (time: string | null | undefined) => {
    if (!time) return null;
    // 如果 time 已经是格式化的字符串（如 "10:30:45"），转换为日期+时间格式
    if (time.match(/^\d{1,2}:\d{2}:\d{2}$/)) {
      const now = new Date();
      return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${time}`;
    }
    try {
