...
export const getSessionExecutions = (sessionId: string) => api.get(`/sessions/${sessionId}/executions`).then(r => r.data);

// ---- Chat ----
export const sendChat = (data: { sessionId: string; message: string; tools?: string[]; skills?: string[] }) =>
  api.post('/chat', data).then(r => r.data);

export const sendChatStream = (data: { sessionId: string; message: string; tools?: string[]; skills?: string[] }, signal?: AbortSignal) =>
  api.post('/chat/stream', data, { signal }).then(r => r.data);
